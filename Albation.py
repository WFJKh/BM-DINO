# =============================================================
# EM‑DINO 5th‑rev  —  Prompt + ROI 版本（支持消融开关）
# =============================================================
# 功能概览：
# 1. 可学习 prompt token (L_p × D) —— 通过 Config.use_prompt 开关控制
# 2. Token 级 ROI Pooling —— 通过 Config.use_roi & Config.use_mask 开关控制
# 3. 掩码分支可选 —— 直接去掉即可得到 RGB‑only 变体
# 4. 冻结/解冻策略可调 —— Config.freeze_backbone, Config.full_ft
# 5. 消融实验只需改 Config 或命令行参数即可
# =============================================================
...
class Config:
    ...
    # ── ablation switches ─────────────────────────────────
    use_prompt    = True      # False -> –Prompt 变体
    prompt_len    = 8         # L_p
    use_roi       = True      # False -> –ROI 变体
    use_mask      = True      # False -> RGB‑only 变体
    freeze_backbone = True    # False -> +Full FT 变体
    full_ft       = False     # True  -> 全部解冻

    # 冻结除最后 k 层外 (仅当 freeze_backbone=True 且 full_ft=False 生效)
    k_unfreeze    = 4
...
class BlastocystDS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = Image.open(mask_path).convert('RGB') if (Config.use_mask and os.path.exists(mask_path)) else Image.new('RGB', img.size)
...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2.0, alpha=None):
        super().__init__()
        self.gamma  = gamma
        self.alpha  = alpha  # 可以是 float 或 1‑D Tensor
    ...
    def forward(self, logits, targets):
        ...
        if isinstance(self.alpha, torch.Tensor):
            at = self.alpha.to(logits.device)[targets]  # 按样本挑选权重
        else:
    ...
    def __init__(self):
        ...
        if not self.cfg.full_ft and self.cfg.freeze_backbone:
            ...
            if self.cfg.k_unfreeze > 0:
                # 兼容 encoder.layer 与 encoder.layers
...
def main():
    ...
    tr_ld = DataLoader(tr_ds, batch_size=Config.batch_size, shuffle=True,  num_workers=Config.num_workers, collate_fn=collate_fn)
    va_ld = DataLoader(va_ds, batch_size=Config.batch_size, shuffle=False, num_workers=Config.num_workers, collate_fn=collate_fn)
    te_ld = DataLoader(te_ds, batch_size=Config.batch_size, shuffle=False, num_workers=Config.num_workers, collate_fn=collate_fn)
    ...
    optim_ = optim.AdamW(filter(lambda p:p.requires_grad, model.parameters()), lr=Config.lr, weight_decay=Config.weight_decay)
    ...
    for ep in range(1, Config.epochs+1):
        ...
        sched_.step()
        # if ep % 20 == 0 or ep==1:
        print(f"Ep {ep:03d} | TrainL {tr_loss:.4f} | ValL {va_loss:.4f} | EXP {va_metrics[0]:.3f} ICM {va_metrics[1]:.3f} TE {va_metrics[2]:.3f}")
